...
function filterPatterns(search_value, $initial_state) {
/* Filter the items based on searched text of > 2 characters */
    ...
    var $rows = $('#pattern-list').children();
    var val = $.trim(search_value).replace(/ +/g, ' ').toLowerCase();

    $rows.show().filter(function() {
        var text = $(this).text().replace(/\s+/g, ' ').toLowerCase().replace('wallpaper','');
...
$(document).ready(function() {
    ...
    $('#search').on('input', function() {
        var val = $(this).val();
        filterPatterns(val, $initial_state);
    });
    ...
    $.get('/tags.json', function(data) {
        $('#search').autocomplete({
            ...
            onSelect: function (suggestion) {
                filterPatterns(suggestion.data, $initial_state);
            }
        });
    });
